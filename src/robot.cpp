...
public:
	Robot::Robot(house& model, size_t max_battery, size_t max_steps, int starting_row, int starting_col)
		: controller(*this), current_battery(max_battery), max_battery(max_battery), current_steps(0), max_steps(max_steps), model(model)
...
private:  // Helper functions
	...
	size_t calculate_dirt(const house& model) const {
		// Todo: implement
		return 10;
	}
};
